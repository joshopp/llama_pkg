...
from experiment_prompts_intent import easy_prompt, easy_target, hard_target, hard_prompt
...
def execute_test(bot, prompt, target, llama=True):
    if llama:
        response, duration = ask_bot(bot, prompt, setup_prompt_intention)
    else:
        response, duration = ask_openai(bot, prompt, setup_prompt_intention)
    ...
    for llm, llm_version in LLMs:
        ...
        for n in range(iterations):
            ...
            if llm == "OpenAI":
                ...
                ariaBot, init_time = initialize_bot(llm, llm_version, setup_prompt_intention)
